...
export const UploadDetails = ({
  ...
  const attemptWeb3StorageLogin = async () => {
    ...
    if (!web3StorageEmailState.value) return toast.error('Please enter a valid email address.')
    ...
    while (true) {
      const res = await toast.promise(
        account.plan.get(),
        {
          loading: 'Waiting for for payment plan to be selected... Please check your inbox.',
          success: 'web3.storage payment plan is acquired.',
          error: 'Failed to acquire payment plan.',
        },
        { duration: 5000, style: { maxWidth: 'none' } },
      )
            ...
            <div className="flex flex-col items-center px-8 w-full">
              ...
              <div className="flex w-full">
                ...
                <Conditional test={uploadService === 'web3-storage'}>
                  <TextInput {...web3StorageEmailState} className="w-1/2" />
                  <Button className="ml-2" isLoading={web3StorageLoginInProgress} onClick={attemptWeb3StorageLogin}>
                    Log In
                  </Button>
                </Conditional>
              </div>
